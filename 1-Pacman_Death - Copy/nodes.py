# This file defines all the nodes in the game grid, which help control Pacman’s and ghosts’ movements.
# nodes.py is used for:
#   Setting up paths and intersections, so characters know where to move.
#   Managing connections between nodes, like tunnels and portals.
#   Making sure characters can smoothly move around corners and choose new directions.

# nodes.py is used in:
#   pacman.py and ghosts.py: to guide movements, so characters follow the grid accurately.
#   constants.py: to get settings for spacing and positioning on the grid.

# In short: nodes.py creates the invisible path network for Pacman and ghosts to move around the maze!
